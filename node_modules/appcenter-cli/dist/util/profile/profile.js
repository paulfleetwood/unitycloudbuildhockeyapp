...
class ProfileImpl {
    get accessTokenId() {
        return token_store_1.tokenStore.get(this.userName)
    ...
    get accessToken() {
        const getter = token_store_1.tokenStore.get(this.userName)
            .catch((err) => token_store_1.tokenStore.get(this.userName, true));
        return getter
...
function saveUser(user, token, environment, tokenSuppliedByUser) {
    return token_store_1.tokenStore.set(user.name, token)
        .then(() => {
