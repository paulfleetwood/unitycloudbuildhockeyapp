...
class CategoryCommand extends command_1.Command {
    ...
    execute() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            interaction_1.out.help(`Usage: ${chalk_1.default.bold(commandTemplate)}`);
            interaction_1.out.help();
    ...
    subCategories(contents) {
        return contents.filter((item) => item[1].isDirectory() && item[0] !== "lib")
            .map((item) => {
            return [chalk_1.default.bold(`    ${item[0]}    `), this.categoryHelp(item[0])];
        });
    }
    categoryCommands(contents) {
        ...
        return contents.filter((item) => item[1].isFile() && /\.[tj]s$/.test(item[0]))
            .map((item) => {
            return [chalk_1.default.bold(`    ${this.commandName(item)}    `), this.commandHelp(item)];
        });
    }
    commandName(item) {
