...
let DownloadBinaryFromDistributionGroupCommand = class DownloadBinaryFromDistributionGroupCommand extends commandline_1.AppCommand {
    ...
    verifyReleaseBelongsToDistributionGroup(client, app, releaseId, distributionGroup) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            let releasesIds;
            try {
                const httpRequest = yield apis_1.clientRequest((cb) => client.releases.listByDistributionGroup(distributionGroup, app.ownerName, app.appName, cb));
    ...
    getReleaseUrl(client, app, releaseId) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const httpRequest = yield apis_1.clientRequest((cb) => client.releases.getLatestByUser(releaseId, app.ownerName, app.appName, cb));
                if (httpRequest.response.statusCode >= 400) {
                    throw httpRequest.response.statusCode;
                }
                else {
    ...
    getLastReleaseUrl(client, app, distributionGroup) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                const httpRequest = yield apis_1.clientRequest((cb) => client.releases.getLatestByDistributionGroup(app.ownerName, app.appName, distributionGroup, "latest", cb));
                if (httpRequest.response.statusCode >= 400) {
    ...
    getDirectoryPath(directoryPath) {
        if (!_.isNil(directoryPath)) {
            ...
            return new Promise((resolve, reject) => {
                MkDirP(normalizedPath, (error) => {
                    if (!_.isNil(error)) {
                        if (error.code === "EEXIST") {
                            reject(commandline_1.failure(commandline_1.ErrorCodes.InvalidParameter, `file ${directoryPath} already exists - directory path is expected`));
                        }
                        else {
                            debug(`Failed to create/access directory ${directoryPath} - ${util_1.inspect(error)}`);
                            reject(commandline_1.failure(commandline_1.ErrorCodes.Exception, `failed to create/access directory ${directoryPath}`));
                        }
                    }
                    else {
    ...
    getFileFullPath(passedFileName, directoryPath, appName, downloadUrl) {
        if (_.isNil(passedFileName)) {
            // creating default file name from app name and "format" query key value of download url
            const ext = "." + Url.parse(downloadUrl, true).query.format;
            const name = appName;
